...
if(isset($_Post['forminscription'])){
	// htmlspecialchars trie les éléments html pour eviter les injections
		...
		$prenom =htmlspecialchars($_POST['penom']);
	...
	if(!empty($_Post['login']) AND !empty($_Post['email']) AND !empty($_Post['email2']) AND !empty($_Post['password']) AND !empty($_Post['password2']) AND !empty($_Post['nom']) AND !empty($_Post['prenom'])){

		$verif_login = $pdo->prepare("SELECT * FROM /*nom de la table*/ WHERE login = ?");
			...
			if ($taillelogin <=50){
				...
				if($email == $email2){
					...
					if(filter_var($email, FILTER_VALIDATE_EMAIL)){
						...
						$verif_mail = $pdo->prepare("SELECT * FROM /*nom de la table*/ WHERE email=?");
						...
						if($mail_exist ==0){
							...
							if($password==$password2){
								// requete préparée
								$insertmbr = $pdo->prepare("INSERT INTO TTV_Client (nomClient, prenomClient, emailClient, loginClient, passwordClient ) VALUES(?, ?, ?, ?, ?)");
